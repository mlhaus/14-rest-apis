...
(function (module) {
  ...
  bookView.initDetailPage = (ctx, next) => {
    ...
    $('#update-btn').on('click', () => {
      let bookId = ($('#update-btn').data('id'));
      page(`/books/${bookId}/update`);
    });

    $('#delete-btn').on('click', () => {
      let bookId = ($('#update-btn').data('id'));
      module.Book.destroy(bookId);
    });
    next();
  }

  bookView.initCreateFormPage = () => {
    $('#create-form input[name="title"]').val('');
    $('#create-form input[name="author"]').val('');
    $('#create-form input[name="isbn"]').val('');
    $('#create-form input[name="image_url"]').val('');
    $('#create-form textarea[name="description"]').val('');
